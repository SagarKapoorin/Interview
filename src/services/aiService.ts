...
export class AIService {
  private questionBank: Omit<Question, 'id'>[] = [
    ...
    },
    {
      ...
      timeLimit: 20,
      expectedAnswer:
        'A React component is a reusable piece of UI. You can create functional or class components.',
    },
    {
    ...
    // Medium Questions (60s each)
    {
      ...
      timeLimit: 60,
      expectedAnswer:
        'Hooks allow functional components to use state and lifecycle methods. Examples: useState, useEffect, useContext.',
    },
    {
      ...
      timeLimit: 60,
      expectedAnswer:
        'Middleware functions execute during request-response cycle. Custom middleware: (req, res, next) => { /* logic */ next(); }',
    },
    {
    ...
    // Hard Questions (120s each)
    {
      ...
      timeLimit: 120,
      expectedAnswer:
        'React uses diffing algorithm to compare virtual DOM trees. Reconciliation efficiently updates only changed parts of real DOM.',
    },
    {
      question:
        'Design a scalable Node.js application architecture for handling high concurrent requests.',
      difficulty: 'Hard',
      timeLimit: 120,
      expectedAnswer:
        'Use clustering, load balancing, caching strategies, database optimization, and microservices architecture.',
    },
    {
      question:
        'Implement a debounce function and explain when you would use it in a React application.',
      difficulty: 'Hard',
      timeLimit: 120,
      expectedAnswer:
        'Debounce delays function execution until after delay period. Useful for search inputs, API calls, resize events.',
    },
  ];
  ...
  generateSummary(answers: Answer[]): { score: number; summary: string } {
    ...
    const easyScore =
      answers.filter((a) => a.difficulty === 'Easy').reduce((sum, a) => sum + a.score, 0) / 2;
    const mediumScore =
      answers.filter((a) => a.difficulty === 'Medium').reduce((sum, a) => sum + a.score, 0) / 2;
    const hardScore =
      answers.filter((a) => a.difficulty === 'Hard').reduce((sum, a) => sum + a.score, 0) / 2;
      ...
      Recommendation: ${
        performanceLevel === 'Excellent'
          ...
          : performanceLevel === 'Good'
            ? 'Promising candidate with some areas to develop'
